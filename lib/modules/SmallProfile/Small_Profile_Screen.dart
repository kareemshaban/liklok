...
class _SmallProfileModalState extends State<SmallProfileModal> {
  ...
  void initState() {
    ...
    if (user!.vips!.length > 0) {
      ...
      final b = vipDesigns!
          .where((element) => int.parse(element.category_id.toString()) == 9)
          .toList()[0]
          .icon;
      setState(() {
        bg = b;
  ...
  void fetchChat() async {
    try {
      chat = await ChatApiService.getChat(user!.id, MyUser!.id);
  ...
  Widget getVipProfileFrame() {
    if (user!.vips!.length > 0) {
      List<Mall>? vipDesigns = user!.vips![0].designs;
      final profile_frame = vipDesigns!
          .where((element) => element.category_id == 8)
          .toList()[0]
          .motion_icon;
      return Transform.translate(
        offset: Offset(0, -160),
        child: SVGAEasyPlayer(
          resUrl: (ASSETSBASEURL + 'Designs/Motion/' + profile_frame),
        ),
      );
    } else {
      return SizedBox();
            ...
            children: [
              ...
              Column(
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      user!.vips!.length > 0 &&
                              designs
                                      .where(
                                        (element) => element.category_id == 10,
                                      )
                                      .toList()
                                      .length >
                                  0
                      ...
                      Row(
                        children: user!.medals!
                            .map((medal) => getMedalItem(medal))
                            .toList(),
                      ),
                      Row(
                        children: user!.relations!
                            .map((medal) => getRelationItem(medal))
                            .toList(),
                      ),
                    ],
                  ),
                  ...
                  Text(
                    user!.status != "" ? user!.status : "Nothing here",
                    style: TextStyle(
                                    ...
                                    child: Column(
                                      children: [
                                        GestureDetector(
                                          ...
                                          child: Image(
                                            image: AssetImage(
                                              'assets/images/profile_gift.png',
                                            ),
                                            width: 70.0,
                                          ),
                                        ),
  ...
  openUserProfile() {
    if (type == 0) {
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (ctx) => InnerProfileScreen(visitor_id: user!.id),
        ),
      );
    } else {
      keepRoom();
  ...
  keepRoom() async {
    ...
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => InnerProfileScreen(visitor_id: user!.id),
      ),
    );
  }
