...
class _ChatBubbleState extends State<ChatBubble> {
  ...
  getRelation() async {
    if (widget.message.contains('YOU CAN DECLINE OR ACCEPT')) {
      String relation = widget.message.substring(
        widget.message.indexOf('RELATION_ID') + 11,
      ...
      bool res = await RelationsServices().getRelation(
        widget.sender_id,
        int.parse(relation),
        widget.reciver_id,
      );
            ...
            child: Column(
              ...
              children: [
                ...
                if (widget.message.contains('YOU CAN DECLINE OR ACCEPT'))
                  Center(
                    child:
                        (widget.sender_id == widget.current_user_id &&
                            isCompleted == 0)
                        ? GestureDetector(
  ...
  showAlertDialog(BuildContext context) {
    ...
    AlertDialog alert = AlertDialog(
      ...
      title: Text(
        "relation_dialog_title".tr,
        style: TextStyle(color: MyColors.primaryColor),
      ),
      content: Text(
        "relation_dialog_msg".tr,
  ...
  acceptRelation() async {
    String relation = widget.message.substring(
      widget.message.indexOf('RELATION_ID'),
      widget.message.length,
    );
    print(relation);
